...
class ContinuityTests(unittest.TestCase):
    ...
    def write_archive(self, session_id: str, exchanges: int = 2) -> Path:
        ...
        for number in range(exchanges):
            rows.extend([
                ...
                codex_message("assistant", f"虚构星图批次 {number} 完成，标记 storybook-{number:04d}。", f"2042-01-01T00:{number:02d}:01Z"),
            ])
    ...
    def test_large_session_reindex_does_not_exceed_sqlite_variable_limit(self) -> None:
        ...
        result = search_index(self.index, "storybook-0429", mode="auto")
    ...
    def test_auto_profile_uses_context_budget(self) -> None:
        ...
        compact = search_index(
            self.index, "storybook-0001", response_profile="compact", mode="auto",
        )
        standard = search_index(
            self.index, "storybook-0001", response_profile="standard", mode="auto",
        )
